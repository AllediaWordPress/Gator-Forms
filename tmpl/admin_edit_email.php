...
<div class="pweb-field">
	...
	<p><strong><?php _e('Administrator', 'pwebcontact'); ?></strong>
		- <?php _e('the owner of website', 'pwebcontact'); ?></p>

	<p><strong><?php _e('User', 'pwebcontact'); ?></strong>
		- <?php _e('a visitor of webiste who wants to contact with website owner', 'pwebcontact'); ?></p>
</div>
...
<div class="pweb-field pweb-field-textarea">
	...
	<div class="pweb-field-control">
		...
		<div
			class="pweb-field-desc"><?php _e('Remeber to create field type of: Send copy to yourself, to use this option.', 'pwebcontact'); ?></div>
	</div>
	<div class="pweb-field-control">
		<?php echo $this->_get_label(array(
		...
		<span
			class="pweb-field-desc"><?php _e('If you have changed format of email then load template again', 'pwebcontact'); ?></span>
	</div>
</div>

<div class="pweb-advanced-options">
	<button type="button" class="button pweb-advanced-options-toggler">
		<i class="glyphicon glyphicon-cog"></i> <span><?php _e('Advanced', 'pwebcontact'); ?></span> <i
...
{files}'
				...
				<div><a href="#" class="pweb-email-tmpl-vars"><?php _e('Show email variables', 'pwebcontact'); ?></a>
				</div>
			</div>
			<div class="pweb-field-control">
				<?php echo $this->_get_label(array(
		...
		<div class="pweb-field pweb-desc">
			...
			<div class="pweb-advanced-options">
				<a href="#" class="pweb-advanced-options-toggler pweb-advanced-options-open">
					<span><?php _e('Universal', 'pwebcontact'); ?></span> <i
						class="glyphicon glyphicon-chevron-down"></i>
				</a>
				...
				<div class="pweb-advanced-options-content">
					<div class="pweb-field-control">
						<p>
							<?php _e('Pageview', 'pwebcontact'); ?>: <code>ga('send', 'pageview',
								document.location.href+'/email-sent');</code><br>
							<?php _e('Event', 'pwebcontact'); ?>: <code>ga('send', 'event', 'Contact', 'E-mail sent',
								document.location.href);</code><br>
							<?php _e('Event with ticket', 'pwebcontact'); ?>: <code>ga('send', 'event', 'Contact',
								'E-mail sent', data.ticket);</code>
						</p>
			...
			<div class="pweb-advanced-options">
				<a href="#" class="pweb-advanced-options-toggler">
					<span><?php _e('Asynchronous', 'pwebcontact'); ?></span> <i
						class="glyphicon glyphicon-chevron-down"></i>
				</a>
				...
				<div class="pweb-advanced-options-content">
					<div class="pweb-field-control">
						<p>
							<?php _e('Pageview', 'pwebcontact'); ?>: <code>_gaq.push(['_trackPageview',
								document.location.href+'/email-sent']);</code><br>
							<?php _e('Event', 'pwebcontact'); ?>: <code>_gaq.push(['_trackEvent', 'Contact', 'E-mail
								sent', document.location.href]);</code><br>
							<?php _e('Event with ticket', 'pwebcontact'); ?>: <code>_gaq.push(['_trackEvent', 'Contact',
								'E-mail sent', data.ticket]);</code>
						</p>
					</div>
				</div>
			...
			<div class="pweb-advanced-options">
				<a href="#" class="pweb-advanced-options-toggler">
					<span><?php _e('Traditional', 'pwebcontact'); ?></span> <i
						class="glyphicon glyphicon-chevron-down"></i>
				</a>
				...
				<div class="pweb-advanced-options-content">
					<div class="pweb-field-control">
						<p>
							...
							<?php _e('Event', 'pwebcontact'); ?>: <code>pageTracker._trackEvent('Contact', 'E-mail
								sent', document.location.href);</code><br>
							<?php _e('Event with ticket', 'pwebcontact'); ?>: <code>pageTracker._trackEvent('Contact',
								'E-mail sent', data.ticket);</code>
						</p>
					</div>
				</div>
...
<div id="pweb-dialog-email-load" title="<?php esc_attr_e('Load email template', 'pwebcontact'); ?>"
     style="display:none">
	<p><?php _e('Current content of email message will be replaced with selected template!', 'pwebcontact'); ?></p>
</div>
<div id="pweb-dialog-email-scheme-load" title="<?php esc_attr_e('Load email scheme', 'pwebcontact'); ?>"
     style="display:none">
...
<div id="pweb-email-tmpl-vars" style="display:none">
	...
	<ul>
		<li><code>{lang:Word}</code>
			- <?php esc_html_e('displays translation of "Word" from language file', 'pwebcontact'); ?></li>
		<li><code>{fields}</code>
			- <?php esc_html_e('displays all fields, each "Label: Value" pair in new line', 'pwebcontact'); ?></li>
		<li><code>{field_alias.label}</code>
			- <?php printf(esc_html__('displays field label, will be translated if present in language file, replace %s with alias of field', 'pwebcontact'), '<code>field_alias</code>'); ?>
		</li>
		<li><code>{field_alias.value}</code>
			- <?php printf(esc_html__('displays field value, replace %s with alias of field', 'pwebcontact'), '<code>field_alias</code>'); ?>
		</li>
		...
		<li><code>{browser}</code> - <?php esc_html_e('User browser name', 'pwebcontact'); ?>.
			...
			<?php if (!is_file(WP_PLUGIN_DIR . '/' . $plugin_file)) : ?>
				<a href="<?php echo wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $plugin), 'install-plugin_' . $plugin); ?>"
				   target="_blank"><?php _e('Install Now'); ?></a>
			<?php elseif (is_plugin_inactive($plugin_file)) : ?>
				<a href="<?php echo wp_nonce_url(self_admin_url('plugins.php?action=activate&plugin=' . $plugin_file), 'activate-plugin_' . $plugin_file); ?>"
				   target="_blank"><?php _e('Activate'); ?></a>
			<?php else : ?>
				<strong class="pweb-text-success"><i class="glyphicon glyphicon-ok-sign"></i> <?php _e('Installed'); ?>
				</strong>
			<?php endif; ?>
			<!-- FREE START -->
			<?php echo $this->_display_badge_pro(); ?>
		...
		<li><code>{mailto_name}</code>
			- <?php esc_html_e('name of recipient selected by User from "Mail to list" field', 'pwebcontact'); ?>
			<!-- FREE START -->
			<?php echo $this->_display_badge_pro(); ?>
			<!-- FREE END -->
		</li>
		...
		<li><code>{files}</code>
			- <?php esc_html_e('links to uploaded files if "Attachment type" set to: "Links to files"', 'pwebcontact'); ?>
			<!-- FREE START -->
			<?php echo $this->_display_badge_pro(); ?>
			<!-- FREE END -->
		</li>

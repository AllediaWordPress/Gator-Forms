...
(function($){
	...
	pwebContact.prototype = (function() {
		...
		defaults:
		{
			...
			modalBackdrop: true,
			modalClose: true,
			modalStyle: 'default', // @deprecated since 2.1
		...
		init: function(options)
		{
            ...
            /*** PRO START ***/
			// enable captcha
			captcha[this.options.id] = { enabled: 0, status: -1, plugin: this.options.captcha };
 ...
 if ($(this.options.selector+'_form input').css('font-size')) {
		...
		initModal: function()
		{
			...
			if (typeof $.fn.modal === 'function')
			{
				...
				this.options.modalClass = 'pwebcontact'+this.options.id+'_modal-open pweb-modal-open'
                    + (this.options.theme ? ' pweb-theme-' + this.options.theme + '-modal' : '')
                    + ' pweb-modal-' + this.options.modalStyle // @deprecated since 2.1
                ;
			...
			if (typeof $.fn.tooltip === 'function')
			{
				...
				.on('show', function(e){
					...
					if (that.options.bootstrap === 3) {
						e.preventDefault();
						$(this).siblings('div.tooltip.in').remove();
					}
					e.stopPropagation();
					if (e.target !== e.currentTarget) return;
		...
		initCalendar: function()
		{
            if (this.options.calendars.length)
            {
                ...
                else if (this.options.debug) this.debug('jQuery UI Datepicker is not loaded');
		...
		initLabelsOverFields: function()
		{
			...
			.focus(function(){
                ...
                var $label = $('#'+$(this).attr('id')+'-lbl').parent();
			...
			if (typeof $.fn.validate === 'function')
			{
				...
				if (this.uploader) {
					$.validator.addMethod('pweb-validate-uploader', function(value, element) {
						return $(element).closest('.pweb-uploader').find('.files .pweb-upload-success,.files .pweb-upload-ready').length;
					}, '');
				}
                /*** PRO END ***/
		...
		resetForm: function()
		{
			if (this.status == 3) // sent
			{
                ...
                // reset captcha
                captcha[this.options.id].status = -1;
				if (captcha[this.options.id].enabled === true && !this.options.redirectURL) {
                    if (captcha[this.options.id].plugin === 'grecaptcha' && typeof grecaptcha !== 'undefined') {
                        var grecaptchaId = $(this.options.selector + '_captcha').data('grecaptchaId');
                        if (typeof grecaptchaId === 'number')
		...
		{
			if (this.options.msgScroll && (this.options.msgPosition == 'before' || this.options.msgPosition == 'after'))
			{
				...
				{
					var msgTopOffset = this.Msg.offset().top,
						msgHeight = this.Msg.height(),
					...
					if (msgTopOffset < winScrollTop || msgTopOffset + msgHeight > winScrollTop + wndHeight)
					{
						...
						if (this.options.msgPosition == 'before') {
							var pos = msgTopOffset - 100;
						}
						else {
							var pos = msgTopOffset + msgHeight + 100 - wndHeight;
						}
						$('html,body').animate({ scrollTop: parseInt(pos) }, 500);
					}
				}
		...
		debug: function(html, code)
		{
			...
			this.displayAlert(html, 'info', 'Gator Forms Debug', false);
		}
	};
